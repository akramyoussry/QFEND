...
class Noise_Layer(layers.Layer):
    ...
    def __init__(self, T, M, K, profile, **kwargs):
        ...
        elif profile==5: # PSD of 1/f + a bump
            alpha       = 1
            S_Z         = 1*np.array([(1/(fq+1)**alpha)*(fq<=15) + (1/16)*(fq>15) + np.exp(-((fq-40)**2)/50)/2 for fq in f[f>=0]]) 
            self.P_temp = tf.constant( np.tile( np.reshape( np.sqrt(S_Z*M/Ts), (1,1,self.M//2) ), (1,self.K,1) ), dtype=tf.complex64)
            self.call   = self.call_1        
...
class VoLayer(layers.Layer):
    ...
    def call(self, x):
        ...
        VO = tf.reduce_mean(VO, axis=1, keepdims=True)
        return VO 
###############################################################################
class quantumTFsim():
    """
    ...
    def __init__(self, T, M, dynamic_operators, static_operators, noise_operators, measurement_operators, initial_states, K=1, waveform="Gaussian", num_pulses=5, distortion=False, noise_profile=0):
        ...
        waveform         : The type of waveform [either "Zero", "Square", or "Gaussian", "input"]
        ...
        # define a dummy input layer needed to generate the control pulses[if not supplied] and noise
        dummy_input = layers.Input(shape=(1,))
        
        # check if the pulses are supplied externally
        if waveform=="input":
            pulse_time_domain = layers.Input( shape=(M,1,len(dynamic_operators)) )
        else:
        ...
        if waveform=="input": # control pulses externally supplied
            self.model   = Model( inputs = [dummy_input, pulse_time_domain], outputs = expectations )
        else:    
            self.model   = Model( inputs = dummy_input, outputs = [pulse_parameters, pulse_time_domain, expectations] )
    ...
    def simulate(self, simulator_inputs, batch_size = 1):
        """
        This method is for running the simulation
